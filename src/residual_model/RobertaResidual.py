...
class RobertaResidualDataset(Dataset):
...
class RobertaResidual(torch.nn.Module):
    ...
    def forward(self, context, labels=None):
        ...
        logits = self.classification_head(outputs[:, 0, :])
    ...
    #                             ).hidden_states[-1]
    #     embs = outputs[:, 0, :]  # Shape: [batch_size, hidden_size]
    ...
    #     # Pass the SEP embeddings through the classification head
    #     logits = self.classification_head(embs)
